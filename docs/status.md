...
| Tests | 678 passing, 38 files | green |
...
| 2 — resident session, full rebuild only | yes | `test/resident.differential.test.ts`: 33 cases, every mutation row comparing a resident generation's rendered bytes against a cold `analyze()` over the same tree |
