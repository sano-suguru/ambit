...
| `unknown` | 解析不能。全てのエフェクトを含みうる（未宣言そのものとの関係は本節末尾の実装注記を参照） |
...
**実装注記（M1、最初の垂直スライスで確定）**: 「未宣言」と「`unknown`」は同じではない。呼び出し先が `@effects` を宣言している場合、呼び出し元はその宣言を信頼して伝播に使う（呼び出し先自身の本体がその宣言と整合するかは、呼び出し先自身の診断で別途検査する — 通常のモジュール型エフェクトシステムの形）。呼び出し先が無宣言の場合は、呼び出し先自身の本体から再帰的に推論した観測エフェクト集合を使って伝播を続ける。`unknown` が実際に生じるのは、動的 `import()` ・`eval`・`any` 型経由の呼び出し・解決できない呼び出し先など、規則6が指す「解析不能」に到達したときのみである。したがって「未宣言」はそれ自体では `unknown` を意味せず、`--coverage`（4.3）が可視化する集計対象であって、伝播規則への入力ではない。宣言済みの呼び出し先の内部に `unknown` が残っていても、それは呼び出し先自身の警告であり、宣言を信頼した呼び出し元には伝播しない。

規則4（コールバックのエフェクト推論）はこの垂直スライスでは未実装で、`unknown` に落としている。同梱スタブ（下記）が実際に生成するのは `network` / `fs_read` / `fs_write` / `process` の4種のみで、`db_read` / `db_write` / `llm` / `env` はこのスライスでは宣言経由でしか解析に入らない。スタブ照合は名前空間 import（`import * as fs from "node:fs"`）とグローバル識別子（`fetch`）のみを解決し、名前付き import（`import { writeFileSync } from "node:fs"`）は識別子名だけになりスタブに一致しないため `unknown` になる。`unknown` のみの診断は警告であり、`ambit check` の終了コードを 0 のままにする（3.4 の「解析失敗を無違反にすり替えない」とは別の話で、こちらは「解析できたが不明」の扱い）。
...
`unknown` は Ambit の中核概念で、未解決の保証範囲を表す。無宣言のコードは `unknown` の代入先ではなく、可能な限り推論を続ける対象である（4.2 実装注記）。`unknown` は推論してもなお解析不能だった経路にのみ生じる。

- 禁止しない。禁止した瞬間に段階導入が成立しない。
- `ambit check --coverage` で、コードベース中の `unknown` 依存の割合と発生箇所を出力する。主要 KPI として扱うが、この数値だけで保証全体を表すとはみなさない。境界宣言への信頼やスタブの信頼レベルも合わせて表示する。**`--coverage` はこの垂直スライスでは未実装。** `AMB-W001`（`unknown` 到達の警告）も、宣言が `pure`（空集合）の関数にしか出ない — `@effects network` のように非空を宣言した関数が `eval` を呼んでも、現状は診断が出ない。
...
**実装注記**: 現時点で実装済みなのは `ambit check <dir> [--format json|text]` のみ（`node src/cli/main.ts` として実行、`ambit` は npm 未公開）。`init` / `run` / `agent` / `stubs` / `sbom`、および `check` の `--coverage` / `--strict` は未実装。
...
上記の表はパッケージ分割の目標であり、現在の実装はまだ分割していない。単一パッケージ内の `src/core/` `src/checker/` `src/cli/` `src/stubs/` ディレクトリで同じ責任分離を表現している。`@ambit/runtime` は未着手で、`src/stubs/` は Node.js 標準モジュールの最小限のエフェクト定義のみを持つ。
