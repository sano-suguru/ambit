...
アダプタがない場合、ケイパビリティ集合は JSDoc と `withAmbit` の二箇所に書かれる。チェッカーは**ソース上の一致のみ**を検査する: 同一ファイルに宣言されたハンドラを包む `withAmbit` のリテラル配列と、そのハンドラの `@capabilities` を集合として比較し、食い違えばエラーにする。動的に組まれた配列、別ファイルのハンドラ、契約を宣言していないハンドラは比較対象外として警告で可視化する（黙って通さない）。ビルド後・バンドル後の対応付け（12 章「契約とハンドラの対応付け」）はこれでは解決していない。
