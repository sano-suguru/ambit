...
- **JSDoc の限界・シンボル識別**: 無名関数、オブジェクトリテラルのメソッド、default export の無名関数。ファイルと宣言経路等の安定表記を定め、コンパイラ内部 ID に依存させない。get/set アクセサは同名で2つの宣言を持つため、宣言経路表記の衝突を避ける表記をどう定めるか（規則5 はアクセサも同じ規則の対象とするが、宣言経路の識別方式は未定）。
