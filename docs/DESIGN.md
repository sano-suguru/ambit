...
アダプタがない場合、ケイパビリティ集合は JSDoc と `withAmbit` の二箇所に書かれる。チェッカーは**ソース上の一致のみ**を検査する: 同一ファイルに宣言されたハンドラを包む `withAmbit` のリテラル配列と、そのハンドラの `@capabilities` を集合として比較し、食い違えばエラーにする。動的に組まれた配列、別ファイルのハンドラ、契約を宣言していないハンドラは比較対象外として警告で可視化する（黙って通さない）。同じ一致検査をアダプタの登録にも適用する（下の決定）。
...
- 2 は、契約データと実行中のハンドラを結ぶ鍵を必要とする。鍵の候補（シンボル ID = ファイルパス + 宣言経路、`fn.name`）はいずれもバンドラとミニファイアが書き換える対象で、ビルドプラグインなしには保存されない。JSDoc からラッパーを注入する変換は §4.5 で Phase 1 の非目標としており、その変換を前提とする方式は今は選べない。
...
- 1 を採らず 2 だけにすると、バンドル後の対応付けは「ビルドプラグインがあれば動く」という条件付きの保証になり、プラグインのない構成では契約が見つからないハンドラだけが並ぶ。
- どちらも採らないと、アダプタは契約を知らないままコンテキストを積めず、`@entrypoint` を宣言したハンドラのケイパビリティは実行時に一切照合されない。

**二重宣言は消えない**

明示登録は契約を JSDoc と `spec` の二箇所に書く。1 を選んだ以上これは消えず、消すには JSDoc からラッパーを注入する変換（§4.5 の Phase 1 非目標、導入には RFC が必要）が要る。したがって `withAmbit` に対して行っている**ソース上の一致検査を、アダプタの登録にも同じ規則で適用する**: `ambitHandler(spec, handler, decode)` の `spec.capabilities` がリテラル配列で、`handler` が同一ファイルの宣言を指す識別子であれば、その JSDoc の `@capabilities` と集合として比較し、食い違えば `AMB-E010`。比較できない形は `AMB-W004` で可視化する。検査対象を増やすのは、二重宣言が残る方式を選んだ代償を黙って払わないためである。

一致検査の対象は `@capabilities` だけではない。二重に書かれる契約は `spec.budget` と `@budget` も同じであり、実行時に効くのは `spec.budget`、ソースが宣言しているのは `@budget` である。`spec.budget` がリテラルオブジェクトで各値がリテラルであれば `@budget` と比較し、`timeMs` / `costUsd` / `llmCalls` / `onExceed` のいずれかが食い違えば `AMB-E011`、片側にしかない限度も食い違いとして扱う。`onExceed` は両側を既定値 `throw` に揃えてから比較する（JSDoc 側は `@budget` の解析時点で既定値が入るので、`spec` 側の省略と比較できる「不在」の状態がない）。二つの半分は独立に判定する: `spec` が片方をリテラルで書き、もう片方を実行時に組むことがあるためで、比較できなかった半分だけを `AMB-W004` で可視化する。`AMB-E010` を拡張せず別 id にしたのは、`AMB-E010` の `contract` フィールドがケイパビリティの文字列（`declared` / `required` / `excess`）であり、予算の食い違いはそこに入れられるものを持たないからである。
...
```json
{
  ...
  "engine": {"name": "typescript-legacy", "version": "5.9.3"}
}
```
...
- **契約とハンドラの対応付け**: 方式（明示登録）、コメントが消えるビルドとバンドル後の届き方、契約が見つからないハンドラの扱い（`runtime.unscoped`）は §4.4「契約とハンドラの対応付け（決定）」で決着した。残るのは (1) 二重宣言（JSDoc と `spec`）を消す自動対応付け — JSDoc からラッパーを注入する変換は §4.5 で Phase 1 の非目標としており、導入には RFC が要る。(2) アダプタを持たない実行経路（Next.js、BullMQ、`worker_threads`、CLI エントリ）の登録手順。(3) `spec` とハンドラが別ファイル・別モジュールに分かれた登録（現在は `AMB-W004` で比較対象外）を、一致検査でどう扱うか。
...
- **JSDoc の限界・シンボル識別**: 無名関数、default export の無名関数、および識別子名を持たないオブジェクトリテラルのメンバ（computed / 文字列 / 数値キー）。ファイルと宣言経路等の安定表記を定め、コンパイラ内部 ID に依存させない。識別子名のメンバについては §5.3 で `オブジェクト名.メンバ名` と定めたが、宣言経路の連結子 `"."` と衝突しうる名前の表記は未定である。get/set アクセサは同名で2つの宣言を持つため、宣言経路表記の衝突を避ける表記をどう定めるか（規則5 はアクセサも同じ規則の対象とするが、宣言経路の識別方式は未定）。
