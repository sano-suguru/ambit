...
- **JSDoc の限界・シンボル識別**: 無名関数、オブジェクトリテラルのメソッド、default export の無名関数。ファイルと宣言経路等の安定表記を定め、コンパイラ内部 ID に依存させない。get/set アクセサは同名で2つの宣言を持つため、宣言経路表記の衝突を避ける表記をどう定めるか（規則5 はアクセサも同じ規則の対象とするが、宣言経路の識別方式は未定）。この未決は呼び出し側にも波及する: 安定した識別子を持たない宣言は呼び出し先としても解決できないため、object literal のメンバを経由した呼び出しは、呼び出し先が `@effects` を宣言していてもその宣言が伝播せず `unknown` になる（現在の挙動は `docs/limitations.md` の Call resolution）。
