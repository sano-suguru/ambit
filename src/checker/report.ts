...
export interface AnalysisResult {
  ...
  /** Per-function authority, the input `ambit diff` compares. */
 ...
 * plus a `ResolvedConfig`: the resident path holds the same
...
/**
 * The `report` phase: diagnostics, authority records, coverage.
 *
 * One function, called by the cold path (`analyze`) and by the resident one,
 * because the resident result must equal a cold run byte for byte. Two
 * compositions of the same diagnostics would be two orders to keep in step,
 * and the first divergence between them would look like an analysis difference
...
export function buildReport(input: ReportInput): AnalysisResult {
  ...
  return {
    ...
    // resident one cannot disagree about the order by forgetting to — the
 ...
 * reached something it could not resolve", which are warnings only until
 * strict mode promotes them. Deliberately not every warning:
 ...
 * inside the globs it lists, so new code can be held to errors while legacy
 * code stays at warnings. The two are a union, so `--strict` on the command
 * line is never narrowed by a config that lists fewer directories.
 *
