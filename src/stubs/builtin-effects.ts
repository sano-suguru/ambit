 ...
 * is the wrong answer when the effect is known exactly. The `env` effect
 * covers the clock and randomness, and `src/stubs/constructors.ts`
 * already reads `new Date()` that way; a `Date.now()` left `unknown` beside it
 * would be the same operation with two verdicts.
 *
 * Same admission rule as the other two tables: names that
