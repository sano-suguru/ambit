...
/**
 * The capability and budget context an entrypoint establishes.
 * `AsyncLocalStorage` holds the context; the blocking itself is the hooks'
 * job, and the two are deliberately kept separate.
 *
 * Nothing here imports `typescript` or anything under `src/checker/`:
 * runtime enforcement is independent of the analysis engine, so
 ...
 * (`runtime.unscoped`). `allow` is the default so that
